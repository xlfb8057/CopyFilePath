...
### 方式一（推荐）：一键安装脚本
双击 **`install.bat`**。它会自动：
1. 把 `CopyPath.exe` 复制到 `%APPDATA%\CopyFilePath\CopyPath.exe`（这是右键菜单真正调用的运行文件，安装后会长期保留）；
2. 向注册表写入右键菜单项（无需管理员）。

### 方式二：手动安装
1. 把 `CopyPath.exe` 复制到 `%APPDATA%\CopyFilePath\CopyPath.exe`（`%APPDATA%` 即 `C:\Users\<你的用户名>\AppData\Roaming`）；
2. 双击 **`install.reg`** 合并到注册表。
...
### 方式一（推荐）：一键卸载脚本
右键 **`uninstall.bat`** → **「以管理员身份运行」**。它会：
1. 删除注册表右键菜单（同时清理本机 `HKCU` 安装项与旧版可能残留的 `HKLM` 项）；
2. 删除持久化的运行文件 `%APPDATA%\CopyFilePath`；
3. 重启资源管理器以刷新菜单。

### 方式二：手动卸载
双击 **`uninstall.reg`** 合并（删除注册表项，无需管理员即可清除本机 `HKCU` 项）。如需连运行文件一起删除，手动删掉 `%APPDATA%\CopyFilePath` 文件夹即可。
...
| `install.bat` | 一键安装脚本（推荐） | ✅ 必需 |
| `uninstall.bat` | 一键卸载脚本（推荐，需管理员） | ✅ 必需 |
